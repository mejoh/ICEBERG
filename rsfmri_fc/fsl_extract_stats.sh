...
# mask index. 
export FSLDIR=/network/iss/apps/software/scit/fsl/6.0.3
export PATH=$PATH:$FSLDIR/share/fsl/bin
export PATH=$PATH:$FSLDIR/bin
source $FSLDIR/etc/fslconf/fsl.sh
export AFNIDIR=/network/iss/apps/software/scit/afni/24.3.06/abin
module load AFNI
DATA=${optD}
...
${AFNIDIR}/whereami -space MNI -spm -atlas Brodmann_Pijn_AFNI -omask ${OUTBN}_cluster_index+tlrc. > ${OUTBN}_cluster_index_a-Brodmann_Pijn_AFNI.txt
${AFNIDIR}/whereami -space MNI -spm -atlas FS.afni.MNI2009c_asym -omask ${OUTBN}_cluster_index+tlrc. > ${OUTBN}_cluster_index_a-FS_afni_MNI2009c_asym.txt
${AFNIDIR}/whereami -space MNI -spm -atlas Julich_MNI2009c -omask ${OUTBN}_cluster_index+tlrc. > ${OUTBN}_cluster_index_a-Julich_MNI2009c.txt
rm ${OUTBN}_cluster_index+tlrc.*

# Coarse visualization of results (horizontal slices over entire brain)
BACKGROUND=/network/iss/cenir/analyse/irm/users/martin.johansson/templateflow/tpl-MNI152NLin6Asym/tpl-MNI152NLin6Asym_res-02_desc-brain_T1w.nii.gz
#BACKGROUND=/opt/fsl/6.0.5/data/standard/MNI152_T1_2mm_brain.nii.gz
STATIMG=${OUTBN}_mask_t
LUT=${FSLDIR}/etc/luts/renderhot.lut
