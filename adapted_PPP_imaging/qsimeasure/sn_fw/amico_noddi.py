...
def amico_noddi(qsiprepdir: str, subid: str):
    ...
    for n, ses in enumerate(sessions, 1):
        ...
        if dwidir.is_dir():
            ...
            continue
        outputdir = (
            qsiprepdir / "derivatives" / "amico_noddi" / f"sub-{subid}" / ses.name
        )
        ...
        # Set up amico
        ae = amico.Evaluation(
            study_path=str(qsiprepdir),
            subject=f"sub-{subid}",
            output_path=str(outputdir),
        )
        # Bval/Bvec scheme
        amico.util.fsl2scheme(
            bvalsFilename=str(bval), bvecsFilename=str(bvec), schemeFilename=str(scheme)
        )
        # Load data
        ae.load_data(
            dwi_filename=str(preproc),
            scheme_filename=str(scheme),
...
def main():
    ...
    amico_noddi(qsiprepdir=args.qsiprepdir, subid=args.participant_label)
