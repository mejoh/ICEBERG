...
def dipy_b0(qsiprepdir, subid, b0thr=100):
    ...
    for n, ses in enumerate(sessions, 1):
        ...
        outputdir = qsiprepdir / "derivatives" / "dipy_b0" / f"sub-{subid}" / ses.name
        ...
        gtab = gradient_table(bvals=bvals, bvecs=bvecs, b0_threshold=b0thr)
        ...
        outputname2 = outputdir / (
            "sub-" + subid + "_" + ses.name + "_dipy-b0mean.nii.gz"
        )
...
def main():
    ...
    parser.add_argument(
        "-b",
        "--b0_threshold",
        help="Volumes with b-values below this threhsold will be used to construct the final b0-image",
        default=100,
    )
    args = parser.parse_args()

    dipy_b0(
        qsiprepdir=args.qsiprepdir, subid=args.subject_label, b0thr=args.b0_threshold
    )
