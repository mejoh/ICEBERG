...
from fsl.data import dtifit as fsldti
...
def dipy_fw(qsiprepdir: str, subid: str):
    ...
    for n, ses in enumerate(sessions, 1):
        ...
        if dwidir.is_dir():
            ...
            continue
        outputdir = qsiprepdir / "derivatives" / "dipy_fw" / f"sub-{subid}" / ses.name

        # Input files
        preproc = list(dwidir.glob("*space-ACPC_desc-preproc_dwi.nii.gz"))[0]
        mask = list(dwidir.glob("*space-ACPC_desc-brain_mask.nii.gz"))[0]
        bval = list(dwidir.glob("*space-ACPC_desc-preproc_dwi.bval"))[0]
        bvec = list(dwidir.glob("*space-ACPC_desc-preproc_dwi.bvec"))[0]
        ...
        if outputdir.is_dir():
            ...
            print(f">>> Removing pre-existing output...")
        ...
        # Model and fit
        print(f">>> Fitting free-water tensor model")
        ...
        dtifw_FW = fwdtifit.f
        dtifw_FA[dtifw_FW > 0.7] = 0
        # dtifw_MD[dtifw_FW > 0.7] = 0
        dtifw_FA[dtifw_FW > 0.7] = 0
        # Save metrics
        print(f">>> Saving to file...")
        save_nifti(
            str(outputdir / ("sub-" + subid + "_" + ses.name + "_dipy-FAc.nii.gz")),
            dtifw_FA,
            affine_dwi,
        )
        save_nifti(
            str(outputdir / ("sub-" + subid + "_" + ses.name + "_dipy-MDc.nii.gz")),
            dtifw_MD,
            affine_dwi,
        )
        save_nifti(
            str(outputdir / ("sub-" + subid + "_" + ses.name + "_dipy-FW.nii.gz")),
            dtifw_FW,
            affine_dwi,
        )

        # Fit alternative model without fw-compartment
        print(f">>> Fitting FSL tensor model")
        dtimodel = dti.TensorModel(gtab, fit_method="WLS")
        dtifit = dtimodel.fit(data_dwi, mask=data_mask)
        dti_FA = dtifit.fa
        dti_MD = dtifit.md
        dti_FA[dtifw_FW > 0.7] = 0
        # dti_MD[dtifw_MD > 0.75] = 0
        print(f">>> Saving to file")
        save_nifti(
            str(outputdir / ("sub-" + subid + "_" + ses.name + "_dipy-FA.nii.gz")),
            dti_FA,
            affine_dwi,
        )
        save_nifti(
            str(outputdir / ("sub-" + subid + "_" + ses.name + "_dipy-MD.nii.gz")),
            dti_MD,
            affine_dwi,
        )
...
#        conffile = list(dwidir.glob("*confounds_timeseries.tsv"))[0]
#        confs = pd.read_csv(conffile, sep="\t")
#        confs.fillna(0, inplace=True)
#        confs = confs.iloc[:, 0:7]
#        confs.to_csv(
#            outputdir / ("sub-" + subid + "_" + ses.name + "_fsl-confs.csv"),
#            sep="\t",
#            header=False,
#        )
#        fsldti.DTIFitTensor(
#            preproc,
#            str(outputdir / ("sub-" + subid + "_" + ses.name + "_fsl")),
#            mask,
#            bvec,
#            bval,
#            w=True,
...
def main():
    ...
    dipy_fw(qsiprepdir=args.qsiprepdir, subid=args.participant_label)
