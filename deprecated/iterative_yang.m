...
%% Input data
indir="/network/iss/cenir/analyse/irm/users/martin.johansson/iceberg/data/bids_dat/derivatives/reg";
...
for i = 1:numel(subs)
    ...
    for j = 1:numel(ses)
        ...
        img_spect = spm_select('FPList', fullfile(indir, subs{i}, ses{j}), '.*trc-123IFPCIT_rec-ChangReo_space-T1w.*_spect.nii');
        c0_csf = spm_select('FPList', fullfile(indir, subs{i}, ses{j}), 'c0_csf.nii');
        c1_low = spm_select('FPList', fullfile(indir, subs{i}, ses{j}), 'c1_low.nii');
        c1_inter = spm_select('FPList', fullfile(indir, subs{i}, ses{j}), 'c1_inter.nii');
        c1_high = spm_select('FPList', fullfile(indir, subs{i}, ses{j}), 'c1_high.nii');
        c2_wm = spm_select('FPList', fullfile(indir, subs{i}, ses{j}), 'c2_wm.nii');
        ...
        if ~isempty(img_spect) && ~isempty(c0_csf) && ~isempty(c1_low) && ~isempty(c1_inter) && ~isempty(c1_high) && ~isempty(c2_wm)
            ...
            [img_corr, ~] = IterativeYang2(Target, labelledROIs, ROIs, PSFsize, PSFsd, ite);
    
            [outdir, bn, ~] = fileparts(img_spect);
            bn = strrep(bn, 'rec-ChangReo', ['rec-ChangReoIY', num2str(ite)]);
...
% - ite: number of iterations (in general ~8 is sufficient)

function [imageiY, iY]=IterativeYang2(Target,labelledROIs,ROIs,PSFsize,PSFsd,ite)
...
%% Initialise values for the iterative Yang algorithm
...
for i = 1:ite
    ...
    % Define a temporary image that will hold mean values at each
