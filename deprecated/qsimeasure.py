...
def qsimeasure(
    ...
    if not subject_label:
        ...
        sub_dirs = [
            qsiprepdir / ("sub-" + label.replace("sub-", "")) for label in subject_label
        ]
    ...
    for n, sub_dir in enumerate(sub_dirs, 1):
        ...
        running = subprocess.run(
            'if [ ! -z "$(qselect -s RQH)" ]; then qstat -f $(qselect -s RQH) | grep Job_Name | grep qsim_; fi',
        ...
        if skip and f"qsim_{sub_id}" in running.stdout.decode():
            print(
                f">>> Skipping already running / scheduled job ({n}/{len(sub_dirs)}): dtigen_{sub_id}"
            )
        else:
            ...
            if not dryrun:
                ...
                if process.returncode != 0:
                    print(
                        f"ERROR {process.returncode}: Job submission failed\n{process.stderr.decode()}\n{process.stdout.decode()}"
                    )
...
def main():
    ...
    parser.add_argument(
        "-p",
        "--participant_label",
        help="Space seperated list of sub-# identifiers to be processed (the sub- prefix can be removed). Otherwise all sub-folders in the bidsfolder will be processed.",
        nargs="+",
    )
    parser.add_argument(
        "-m",
        "--dti-method",
        help="Method used to generate output. Choose from: dipy_fw, amico_noddi, dipy_b0 (default: dipy_fw).",
    )
    parser.add_argument(
        "-f", "--force", help="Overwrite pre-existing output.", action="store_true"
    )
    parser.add_argument(
        "-d",
        "--dryrun",
    ...
    parser.add_argument(
        "-i",
        "--ignore",
        help="Submit jobs when there are already jobs with the same name in queue or running.",
        action="store_false",
    )
